...
class DDPG:
  def __init__(self, config):
    self.config = config
    self.online_value_network = ValueNetwork(
    ...
    self.online_policy_network = PolicyNetwork(
      config.state_dim, config.action_dim, [256, 256], config.a_lb, config.a_ub
    )
    ...
    self.value_optimizer = optim.Adam(
      self.online_value_network.parameters(), lr=self.config.lr
    )
...
if __name__ == "__main__":
  config = Config(state_dim=3, action_dim=1, a_lb=[-2.0], a_ub=[2.0])
  ddpg = DDPG(config)
  train(config, ddpg)
  simulate(ddpg)
