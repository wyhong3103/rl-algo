...
class Sarsa:
  ...
  def update(self, reward, r, c, d, a, n_r, n_c, n_d, n_a):
    self.q[r][c][d][a] = self.q[r][c][d][a] + self.alpha * (
      reward + self.gamma * self.q[n_r][n_c][n_d][n_a] - self.q[r][c][d][a]
    )
...
class QLearning:
  ...
  def update(self, reward, r, c, d, a, n_r, n_c, n_d):
    self.q[r][c][d][a] = self.q[r][c][d][a] + self.alpha * (
      reward
      + self.gamma * self.q[n_r][n_c][n_d][self.get_max_action(n_r, n_c, n_d)]
      - self.q[r][c][d][a]
    )
...
def run_episode(env, policy, train=True):
  ...
  for _ in range(10000):
    ...
    if train:
      if algo in [
        "watkins_q_with_eligibility_trace",
        "sarsa_with_eligibility_trace",
        "sarsa",
      ]:
