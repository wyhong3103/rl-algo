...
class A3C:
  ...
  def optimize(self, local_policy_model, local_value_model, rewards, logpas, values, entropys):
    ...
    local_value_model.load_state_dict(
      self.global_value_model.state_dict()
    )
    ...
    for episode in range(self.total_episodes):
      ...
      while not terminated:
        ...
        if current_step == self.n or terminated:
          self.optimize(local_policy_model, local_value_model, rewards, logpas, values, entropys)
          current_step = 0
          rewards = []
          logpas = []
          values = []
      ...
      if rank == 0:
        ...
        moving_avg.append(
          (
            sum(global_rewards) + 
            sum(returns[-min(window_len-1, len(returns)):])
          ) / 
